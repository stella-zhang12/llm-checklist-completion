...
def analyze_file(filename, prompt_template, model_id, output_path):
    ...
    # Double check inside thread just in case of race conditions (rare)
...
def main():
    ...
    for folder_name, model_id in MODELS_CONFIG.items():
        ...
        for filename, template in base_tasks:
            # Construct the exact path where the result SHOULD be
            output_json_name = os.path.splitext(filename)[0] + ".json"
            output_path = os.path.join(out_dir, output_json_name)
            
            # PRE-FLIGHT CHECK: Does it exist?
            if os.path.exists(output_path):
                skipped_count += 1
                continue # Skip adding to job list
    ...
    # We create bars only for models that actually have work, but simpler to create all 3 
    # so the layout stays consistent.
    for i, model_name in enumerate(MODELS_CONFIG.keys()):
        # Calculate how many jobs are specifically for this model to set the total bar correctly
